...
### 為什麼選擇 Qdrant 作為向量資料庫？

| 特性 | Qdrant | Pinecone | Chroma | Milvus |
|------|--------|----------|--------|--------|
| 開源 | ✅ 完全開源 | ❌ 閉源 SaaS | ✅ 開源 | ✅ 開源 |
| 自架 | ✅ Docker 一行 | ❌ 只有雲端 | ✅ 輕量 | ✅ 但很重 |
| 免費 | ✅ 完全免費 | ❌ 要錢 | ✅ 免費 | ✅ 免費 |
| 效能 | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐⭐⭐ |
| 易用性 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐ |

**選擇原因：**
1. **Docker 一行搞定** - 符合課程 Docker 部署方式
2. **完全免費 + 開源** - 資料在自己手上，不用註冊帳號
3. **Python SDK 好用** - `qdrant-client` 簡潔易懂
4. **REST API 完整** - 方便 debug 和測試
5. **業界認可** - Notion, Podimo 等公司使用

### 為什麼選擇 Ollama 作為主要 LLM？

| 特性 | Ollama (本地) | Groq (雲端) | OpenAI (雲端) |
|------|--------------|-------------|---------------|
| 成本 | $0 | 免費額度 | 付費 |
| 延遲 | 較高 | 極低 | 低 |
| 隱私 | ✅ 資料不外傳 | ❌ 送到雲端 | ❌ 送到雲端 |
...
**選擇原因：**
1. **零成本** - 本地運行，完全免費
2. **與 KDAI 一致** - KDAI 也使用 Ollama
3. **隱私友好** - 資料不離開本機
4. **學習價值** - 了解本地 LLM 部署

**Fallback 策略：** Ollama (失敗) → Groq (失敗) → OpenAI
...
| 效能 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ |
| Async 支援 | ✅ 原生 | ❌ 需擴展 | ❌ 需擴展 |
| 型別驗證 | ✅ Pydantic | ❌ 需擴展 | ❌ 需擴展 |
| 自動文檔 | ✅ Swagger UI | ❌ 需擴展 | ❌ 需擴展 |
| 學習曲線 | 低 | 低 | 高 |

**選擇原因：**
1. **原生 Async** - 適合 LLM streaming 和 SSE
2. **Pydantic 驗證** - 自動型別檢查
3. **自動 API 文檔** - `/docs` 自動產生 Swagger UI
4. **AI 生態系** - 大多數 AI 專案使用 FastAPI
